...
def get_dataset(dir, name, roll, conf, user_id):
    ...
    if name == "Shakespeare":
        ...
        if conf["non_iid"] == "iid":
            ...
            if roll == "s":
                ...
                x = (
                    data_conf["user_data"][usr1]["x"]
                    + data_conf["user_data"][usr2]["x"]
                )
                y = (
                    data_conf["user_data"][usr1]["y"]
                    + data_conf["user_data"][usr2]["y"]
                )
        ...
        train_dataset = datasets.EMNIST(
            dir, train=True, download=True, transform=transform_train, split="byclass"
        )
        eval_dataset = datasets.EMNIST(
            dir, train=False, transform=transform_test, split="byclass"
        )
    elif name == "cifar10":
        transform_train = transforms.Compose(
            [
    ...
    if roll == "s":
        ...
        for label, indices in label_to_indices.items():
            eval_loader_label = torch.utils.data.DataLoader(
                eval_dataset,
                batch_size=conf["batch_size"],
                pin_memory=pin,
        ...
        if user_id == 0:
            if conf["redistribution"] == "y":
                ...
                if conf["non_iid"] == "HeteroDiri":
                    partition = np.random.dirichlet(
                        [conf["alpha"]] * conf["no_models"], len(label_to_indices)
                    )
                    label_partition = {}
                    for label, indices in label_to_indices.items():
                    ...
                    for label, parti in label_partition.items():
                        ...
                        Dataset_label_indices[label] = np.split(
                            label_indices[label], par
                        )
                    ...
                    shards = [
                        squence[i : i + step] for i in range(0, len(squence), step)
                    ]
                    ...
                    for i in range(conf["no_models"]):
                        client_labels = random.sample(
                            labels, int(conf["alpha"] * len(labels))
                        )
                        client_data = []
                    ...
                    for i in range(conf["no_models"]):
                        ...
                        for j in range(n_class):
                            ...
                            point_indices[j] += n
                            data_indices += label_to_indices[j][
                                c_partition[j][0] : c_partition[j][1]
                            ]
                    ...
                    for i in range(conf["no_models"]):
                        data_indices = random.sample(
                            list(range(0, len(train_dataset))),
                            int(conf["alpha"] * len(train_dataset)),
                        )
